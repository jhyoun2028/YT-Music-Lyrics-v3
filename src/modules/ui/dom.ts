...
let overlayKeydownHandler: ((event: KeyboardEvent) => void) | null = null;

const OVERLAY_ID = "aml-overlay";
const LINES_WRAPPER_ID = "aml-lines-wrapper";
const OFFSET_DISPLAY_ID = "aml-offset-display";
const DEBUG_ID = "aml-debug";

function getOverlay(): HTMLElement | null {
  return document.getElementById(OVERLAY_ID);
}

function getOffsetStorageKey(videoId: string): string {
  return `aml_offset_${videoId}`;
}

function getCurrentOffsetStorageKey(): string | null {
  if (!AppState.lastVideoId) {
    return null;
  }
  return getOffsetStorageKey(AppState.lastVideoId);
}

function formatOffsetMs(offsetSeconds: number): string {
  return `${Math.round(offsetSeconds * 1000)}ms`;
}

function updateOffsetDisplay(): void {
  const display = document.getElementById(OFFSET_DISPLAY_ID);
  if (display) {
    display.textContent = formatOffsetMs(AppState.userOffset);
  }
}

function saveCurrentOffset(): void {
  const storageKey = getCurrentOffsetStorageKey();
  if (!storageKey) {
    return;
  }
  localStorage.setItem(storageKey, AppState.userOffset.toFixed(1));
}

function setUserOffset(nextOffset: number): void {
  AppState.userOffset = Number(nextOffset.toFixed(1));
  saveCurrentOffset();
  updateOffsetDisplay();
}

function detachOverlayKeyHandler(): void {
...
export function createOverlay(): HTMLElement {
  ...
  overlay.innerHTML = `
    <div class="aml-bg-tint" id="aml-bg-tint"></div>
    <div class="aml-left" id="aml-left">
      <img class="aml-album-art" id="aml-album-art" />
      <div class="aml-song-title" id="aml-song-title"></div>
      <div class="aml-song-artist" id="aml-song-artist"></div>
    </div>
    <div class="aml-content" id="aml-content">
      <div class="aml-lines-wrapper" id="aml-lines-wrapper">
        <div class="aml-spacer-top"></div>
        <div class="aml-spacer-bottom"></div>
      </div>
    </div>
    <button class="aml-close" id="aml-close">&times;</button>
    <div class="aml-offset-controls" id="aml-offset-controls">
      <button class="aml-offset-btn" id="aml-offset-minus">-0.1s</button>
      <span class="aml-offset-display" id="aml-offset-display">0ms</span>
      <button class="aml-offset-btn" id="aml-offset-plus">+0.1s</button>
    </div>
    <div class="aml-toolbar" id="aml-toolbar">
      <div class="aml-toolbar-progress" id="aml-toolbar-progress"></div>
      <div class="aml-toolbar-inner">
        <div class="aml-toolbar-controls">
          <button class="aml-btn-prev" id="aml-btn-prev">&#x23EE;</button>
          <button class="aml-btn-play" id="aml-btn-play">&#x23F8;</button>
          <button class="aml-btn-next" id="aml-btn-next">&#x23ED;</button>
        </div>
        <div class="aml-toolbar-info">
          <div class="aml-toolbar-title" id="aml-toolbar-title"></div>
          <div class="aml-toolbar-artist" id="aml-toolbar-artist"></div>
        </div>
        <div class="aml-toolbar-right" id="aml-toolbar-time">0:00 / 0:00</div>
      </div>
    </div>
    <div class="aml-debug aml-hidden" id="aml-debug"></div>
  `;
  ...
  const closeOverlay = () => {
    hideOverlay();
    AppState.closedByUser = true;
    overlay.remove();
    detachOverlayKeyHandler();
  };

  overlay.querySelector("#aml-close")?.addEventListener("click", closeOverlay);
  overlay.querySelector("#aml-offset-minus")?.addEventListener("click", () => {
    setUserOffset(AppState.userOffset - 0.1);
  });
  overlay.querySelector("#aml-offset-plus")?.addEventListener("click", () => {
    setUserOffset(AppState.userOffset + 0.1);
  });

  overlay.querySelector("#aml-btn-play")?.addEventListener("click", () => {
    (document.querySelector(".play-pause-button") as HTMLElement | null)?.click();
  });
  overlay.querySelector("#aml-btn-prev")?.addEventListener("click", () => {
    (document.querySelector(".previous-button") as HTMLElement | null)?.click();
  });
  overlay.querySelector("#aml-btn-next")?.addEventListener("click", () => {
    (document.querySelector(".next-button") as HTMLElement | null)?.click();
  });
  ...
  overlay.addEventListener(
    ...
    event => {
      ...
      const lineElement = target.closest(".aml-line") as HTMLElement | null;
      if (!lineElement) {
        return;
      }

      const seekTime = Number.parseFloat(lineElement.dataset.time || "0");
...
export function updateOverlaySongInfo(title: string, artist: string, albumArtUrl: string): void {
  const overlay = getOverlay();
  if (!overlay) {
    return;
  }

  const songTitle = overlay.querySelector("#aml-song-title");
  const songArtist = overlay.querySelector("#aml-song-artist");
  const toolbarTitle = overlay.querySelector("#aml-toolbar-title");
  const toolbarArtist = overlay.querySelector("#aml-toolbar-artist");
  const albumArt = overlay.querySelector("#aml-album-art") as HTMLImageElement | null;
  const bgTint = overlay.querySelector("#aml-bg-tint") as HTMLElement | null;
  ...
  if (toolbarArtist) toolbarArtist.textContent = artist;

  if (albumArt) {
    albumArt.src = albumArtUrl;
    albumArt.alt = `${title} - ${artist}`;
  }
  if (bgTint) {
    bgTint.style.backgroundImage = albumArtUrl ? `url('${albumArtUrl}')` : "";
  }
}
...
export function updateToolbar(currentTime: number, duration: number, isPlaying: boolean): void {
  const overlay = getOverlay();
  if (!overlay) {
    return;
  }

  const progress = overlay.querySelector("#aml-toolbar-progress") as HTMLElement | null;
  const timeText = overlay.querySelector("#aml-toolbar-time") as HTMLElement | null;
  const playButton = overlay.querySelector("#aml-btn-play") as HTMLElement | null;

  const safeDuration = Number.isFinite(duration) && duration > 0 ? duration : 0;
  const safeCurrentTime = Number.isFinite(currentTime) ? Math.max(0, currentTime) : 0;
  const ratio = safeDuration > 0 ? Math.min(1, safeCurrentTime / safeDuration) : 0;

  if (progress) {
    progress.style.width = `${ratio * 100}%`;
  }
  if (timeText) {
    timeText.textContent = `${formatTime(safeCurrentTime)} / ${formatTime(safeDuration)}`;
  }
  if (playButton) {
...
export function toggleDebugOverlay(): void {
  const debug = document.getElementById(DEBUG_ID);
  if (!debug) {
    return;
  }
  debug.classList.toggle("aml-hidden");
}
/**
...
export function createLyricsWrapper(): HTMLElement {
  ...
  const linesWrapper = document.getElementById(LINES_WRAPPER_ID);
  const spacerBottom = linesWrapper?.querySelector(".aml-spacer-bottom");
  if (linesWrapper && spacerBottom) {
    linesWrapper.insertBefore(wrapper, spacerBottom);
  } else if (linesWrapper) {
...
function setOverlayAlbumArt(src: string): void {
  const albumArt = document.getElementById("aml-album-art") as HTMLImageElement | null;
  const bgTint = document.getElementById("aml-bg-tint");
  if (albumArt) {
    albumArt.src = src;
  }
  if (bgTint) {
    bgTint.style.backgroundImage = src ? `url('${src}')` : "";
  }
}
...
export function addThumbnail(smallThumbnail: ThumbnailElement): void {
  ...
  if (proxy.complete) {
    ...
    if (lastLoadedThumbnail !== smallThumbnail) {
      ...
      setOverlayAlbumArt(smallThumbnail.url);
    }
...
export function injectSongAttributes(title: string, artist: string): void {
  ...
  const albumArtUrl =
    (document.getElementById("blyrics-img") as HTMLImageElement | null)?.src ||
    (document.querySelector("#thumbnail>#img") as HTMLImageElement | null)?.src ||
    "";
  updateOverlaySongInfo(title, artist, albumArtUrl);
}
