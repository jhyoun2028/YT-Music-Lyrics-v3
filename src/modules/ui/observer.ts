...
let hasInitializedOverlayKeyboardHandler = false;
...
function loadSongOffset(videoId: string): void {
  const savedOffset = localStorage.getItem(`aml_offset_${videoId}`);
  AppState.userOffset = savedOffset ? Number.parseFloat(savedOffset) || 0 : 0;
}

function getCurrentAlbumArtUrl(): string {
  return (
...
export function initializeLyrics(): void {
  ...
  document.addEventListener("aml-toolbar-update", event => {
    if (!AppState.overlayVisible) {
      return;
    }
    const detail = (event as CustomEvent<{ currentTime: number; duration: number; playing: boolean }>).detail;
    if (!detail) {
      return;
    }
    updateToolbar(detail.currentTime, detail.duration, detail.playing);
  });

  document.addEventListener("blyrics-send-player-time", (event: Event) => {
    const customEvent = event as CustomEvent<PlayerDetails>;
    ...
    if (currentVideoId !== AppState.lastVideoId || currentVideoDetails !== AppState.lastVideoDetails) {
      ...
      AppState.closedByUser = false;
      loadSongOffset(detail.videoId);
      if (!AppState.closedByUser) {
        showOverlay();
      }
      updateOverlaySongInfo(detail.song, detail.artist, getCurrentAlbumArtUrl());
      ...
      getSongMetadata(detail.videoId, 250, abortController.signal).then(async songMetadata => {
        ...
        if (songMetadata) {
          ...
          updateOverlaySongInfo(detail.song, detail.artist, songMetadata.smallThumbnail.url);
        } else {
          showYtThumbnail();
          updateOverlaySongInfo(detail.song, detail.artist, getCurrentAlbumArtUrl());
        }
...
export function setupOverlayKeyboardHandler(): void {
  if (hasInitializedOverlayKeyboardHandler) {
    return;
  }
  hasInitializedOverlayKeyboardHandler = true;
