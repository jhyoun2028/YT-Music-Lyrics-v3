...
import { hideAdOverlay, isAdPlaying, showAdOverlay, updateToolbar } from "@modules/ui/dom";
...
interface YtmPlayerElement extends HTMLElement {
  getDuration?: () => number;
}
...
export function animationEngine(currentTime: number, eventCreationTime: number, isPlaying = true, smoothScroll = true) {
  ...
  try {
    ...
    const overlayLinesWrapper = document.getElementById("aml-lines-wrapper") as HTMLElement | null;
    const overlayContent = document.getElementById("aml-content") as HTMLElement | null;
    const isOverlayMode = AppState.overlayVisible && !!overlayLinesWrapper;
    const viewportHeight = isOverlayMode
      ? (overlayContent?.getBoundingClientRect().height ?? tabRendererHeight)
      : tabRendererHeight;
    ...
    const activeIndex = animEngineState.selectedElementIndex;
    const PROXIMITY_RANGE = 6;
    const startIdx = Math.max(0, activeIndex - PROXIMITY_RANGE);
    const endIdx = Math.min(lines.length - 1, activeIndex + PROXIMITY_RANGE);

    for (let i = startIdx; i <= endIdx; i++) {
      const el = lines[i].lyricElement;
      el.classList.remove(...PROXIMITY_CLASSES);
    ...
    if (activeElems.length > 0) {
      const activeLine = activeElems[activeElems.length - 1];
      activeLine.parts.forEach(part => {
        const wordEl = part.lyricElement;
        wordEl.classList.remove("aml-word-active", "aml-word-past");
        if (currentTime >= part.time && currentTime < part.time + part.duration) {
          wordEl.classList.add("aml-word-active");
        } else if (currentTime >= part.time + part.duration) {
          wordEl.classList.add("aml-word-past");
        }
      });
    }
    ...
    if (animEngineState.scrollResumeTime < Date.now() || animEngineState.scrollPos === -1) {
      ...
      const scrollPosOffset = viewportHeight * SCROLL_POS_OFFSET_RATIO.getNumberValue();
      ...
      scrollPos = Math.max(scrollPos, lastActiveLyric.position - viewportHeight + lastActiveLyric.height);
      ...
      if (ENABLE_DEBUG_RENDER.getBooleanValue()) {
        ...
        if (ctx) {
          ...
          ctx.fillStyle = "blue";
          ctx?.fillText("visible bottom", 0, scrollTop + viewportHeight);
          ctx?.beginPath();
          ctx?.moveTo(40, scrollTop + viewportHeight);
          ctx?.lineTo(1000, scrollTop + viewportHeight);
          ctx.stroke();
      ...
      if (!isOverlayMode && scrollTop === 0 && !animEngineState.doneFirstInstantScroll) {
      ...
      if (animEngineState.wasUserScrolling || newLyricSelected || animEngineState.queuedScroll) {
        if (Date.now() > animEngineState.nextScrollAllowedTime) {
          ...
          if (smoothScroll && Math.abs(scrollTop - scrollPos) > 2) {
            if (isOverlayMode && overlayLinesWrapper) {
              const scrollTime = getCSSDurationInMs(lyricsElement, "transition-duration");
              overlayLinesWrapper.style.transition = "";
              overlayLinesWrapper.style.transform = `translate3d(0px, ${-scrollPos}px, 0px)`;
          ...
          if (isOverlayMode && overlayLinesWrapper) {
            overlayLinesWrapper.style.transform = `translate3d(0px, ${-scrollTop}px, 0px)`;
          } else {
    ...
    if (AppState.overlayVisible) {
      const player = document.getElementById("movie_player") as YtmPlayerElement | null;
      const duration = player?.getDuration?.() ?? 0;
      updateToolbar(currentTime, duration, isPlaying);
    }
